...
| `/api/v1/songs-crawl` | `endpoints/songs_crawl.py` | Song-master crawler (textage): `/targets`, `/preview` diagnostics (public, read-only). |
| `/api/v1/difficulty-crawl` | `endpoints/difficulty_crawl.py` | Difficulty-table crawler: `/targets`, `/preview` (**the weekly scheduler lives in `app/services/difficulty_crawl/scheduler.py`**). |
| `/api/v1/admin` | `endpoints/admin.py` | Admin API for the separate admin FE: manual crawl jobs (`/crawl/jobs`) + schedule get/put (`/crawl/schedule`). Auth + `ADMIN` role. |
...
One weekly APScheduler job (`app/services/difficulty_crawl/scheduler.py`, default Mon 05:00 Asia/Seoul) runs `run_song_sync` (`services/songs_crawl` → `sync_song_master` RPC) **then** `run_table_sync` (`services/difficulty_crawl` → `sync_table_result` RPC), in that order — the song master must land before table mapping. Ordering is guaranteed in code, never by staggered cron times. A failed song-master sync does not block the table sync (it proceeds on last week's master). The table-read endpoints (`endpoints/tables.py` → `crud/crud_tables.py`) read back from Supabase (public RLS read). **There is intentionally no manual sync endpoint in the crawl modules** — manual runs go through `POST /api/v1/admin/crawl/jobs` only (RPCs are service-role only: `sync_song_master`, `sync_table_result`).

### Admin — crawl jobs & schedule (Redis-backed)
`app/services/admin` (`jobs.py` runner, `store.py` Redis persistence); the `AdminUser` guard is in `app/api/deps.py`, the router in `app/api/v1/endpoints/admin.py`. Both manual and scheduled runs go through `jobs.create_job`/`execute_job`, which checkpoints per step (`song_sync` → `table_sync`) in Redis: on startup `resume_interrupted_job` re-runs unfinished steps of a job that was cut off mid-run (safe because crawl→RPC is upsert/full-replace, i.e. idempotent). One job at a time — the "current job" key in Redis is the lock; stale RUNNING leftovers are auto-failed by the next `create_job`. Schedule changes via `PUT /admin/crawl/schedule` are stored in Redis and **override** the `CRAWL_SCHEDULE_*` env defaults across restarts (`scheduler.get_effective_schedule`). If Redis is down, the weekly cron still crawls (untracked fallback in `run_weekly_sync`); admin endpoints return 503. Admin access = valid Supabase token **and** `app_metadata.role == "ADMIN"` on the Supabase account (granted via SQL on `auth.users.raw_app_meta_data` — see README; takes effect on next token refresh). Roles live in `app/schemas/user.py` (`UserRole` + `ROLE_LEVELS`, hierarchy-based so higher roles include lower ones); protect endpoints with `require_role(UserRole.X)` from `app/api/deps.py`. Unknown/missing role ⇒ `USER`.

### Crawler plugin pattern
To add a difficulty table you normally touch **only** `app/services/difficulty_crawl/crawlers/` and `TABLE_CRAWL_TARGETS` — schema and sync code stay untouched:
...
The two crawl subsystems have **separate registries and target lists**: `SONG_CRAWL_TARGETS` (`services/songs_crawl`, `SongMasterResult`) vs `TABLE_CRAWL_TARGETS` (`services/difficulty_crawl`, `TableDef`/`TableResult`). Adding a difficulty table touches only `app/services/difficulty_crawl/crawlers/` + `TABLE_CRAWL_TARGETS`; adding a song source touches only `app/services/songs_crawl/crawlers/` + `SONG_CRAWL_TARGETS`.
