...
동기(블로킹) 함수이므로 비동기 컨텍스트(app/services/iidx/admin/store.py)에서는
asyncio.to_thread로 호출한다. 스키마: supabase/migrations/20260803000000_baseline.sql
(crawl_targets/crawl_schedules 는 iidx 스키마의 service_role 전용 테이블 —
get_supabase_iidx 클라이언트로 접근, public read 정책 없음).
...
def list_targets() -> dict[str, dict]:
    ...
    res = get_supabase_iidx().table("crawl_targets").select("*").execute()
...
def upsert_target(target_key: str, config: dict) -> None:
    ...
    get_supabase_iidx().table("crawl_targets").upsert(row).execute()


def delete_target(target_key: str) -> None:
    get_supabase_iidx().table("crawl_targets").delete().eq("target_key", target_key).execute()


def list_schedules() -> dict[str, dict]:
    """target_key -> {enabled, triggers}."""
    res = get_supabase_iidx().table("crawl_schedules").select("*").execute()
...
def upsert_schedule(target_key: str, config: dict) -> None:
    get_supabase_iidx().table("crawl_schedules").upsert({
        "target_key": target_key,
...
def delete_schedule(target_key: str) -> None:
    get_supabase_iidx().table("crawl_schedules").delete().eq("target_key", target_key).execute()
