"""사용자 프로필 조회/수정(웹 클라이언트) API 모델 — user_profiles 테이블 기준.

dj_name/dj_id/profile_image_url은 요청 모델에 없다 — 사용자가 API로
직접 수정하지 않는 필드다(DB 직접 등록, 추후 북마크릿 데이터 갱신 파이프라인이
채운다).
"""
...
class ProfileUpdateRequest(BaseModel):
    """PATCH /web/profile/me 요청 — 명시적으로 보낸 필드만 갱신한다(부분 업데이트).
