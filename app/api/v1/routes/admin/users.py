...
크롤 동기화/스케줄 API는 /api/v1/iidx/crawl/* 로 통합되어 있다(app/api/v1/endpoints/crawl.py).
...
from app.crud.account import bans as crud_bans, profiles as crud_profiles, users as crud_users
from app.schemas.account.admin import (
    AdminUserDetail,
...
from app.schemas.account.user import ROLE_LEVELS, UserRole
...
async def ban_user(user_id: str, body: BanRequest, admin: AdminUser):
    ...
    # banned_by는 profiles(id)를 참조하는 uuid이므로 이메일이 아니라 admin.id를 넘긴다
    ban = await asyncio.to_thread(
        crud_bans.create_ban, user_id, body.reason, ban_until_str, admin.id
    )
...
async def unban_user(user_id: str, admin: AdminUser):
    """현재 활성 접근 제한을 해제한다."""
    lifted = await asyncio.to_thread(crud_bans.lift_active_ban, user_id, admin.id)
    if not lifted:
        raise HTTPException(
