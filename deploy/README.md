...
`development` 푸시는 개발 API를 중지하고 새 이미지를 빌드한 뒤 다시 시작하며, 기존 Nginx Proxy Manager의 `iinfo-dx-api-dev.forestlee.me` 경로를 확인합니다. `main` 푸시는 운영 API의 대기 슬롯을 빌드하고 건강 상태를 확인한 다음 `iinfo-dx-api.forestlee.me` 프록시 대상을 전환합니다. 이전 슬롯은 30초 동안 유지해 기존 요청을 처리한 뒤 중지합니다.

## 서버 배치

- 저장소: `/home/forestlee/deploy/iinfo-dx-backend/repo` (Actions가 HTTPS로 클론하고 해당 커밋을 체크아웃)
- 개발 설정: `/home/forestlee/deploy/iinfo-dx-backend/env/development.env`
- 운영 설정: `/home/forestlee/deploy/iinfo-dx-backend/env/production.env`
- NPM API 자격증명: `/home/forestlee/deploy/iinfo-dx-backend/npm-credentials.json` (권한 `600`)
- 상태 파일: `/home/forestlee/deploy/iinfo-dx-backend/active-slot`
...
운영 API는 NPM의 `iinfo-dx-api.forestlee.me` 호스트가 현재 슬롯(`iinfo-dx-production-blue` 또는 `iinfo-dx-production-green`)의 8000번 포트로 직접 전달합니다. 개발 API는 `iinfo-dx-api-dev.forestlee.me`에서 `iinfo-dx-development-api:8000`으로 전달하며 서버 내부의 `127.0.0.1:18081`로도 확인할 수 있습니다. NPM API 자격증명 파일은 `{"identity":"관리자 이메일","secret":"관리자 비밀번호"}` 형식입니다. 프록시 호스트가 없으면 기존 Let's Encrypt `*.forestlee.me` 인증서로 생성합니다.

NPM 호스트의 고급 설정에는 `/internal`, 관리자 경로, 크롤 작업·스케줄·대상 상세 경로의 공개 접근 차단 규칙을 추가합니다. 대상 목록의 쓰기 요청은 앱의 ADMIN 인증으로 보호됩니다.

ForestLee의 `~/services/stacks/cloudflare-ddns/compose.yaml`에 운영·개발 도메인을 추가하고 Cloudflare A 레코드를 관리합니다.
...
curl --resolve iinfo-dx-api-dev.forestlee.me:443:127.0.0.1 \
  https://iinfo-dx-api-dev.forestlee.me/api/v1/health
curl -fsS http://127.0.0.1:18081/api/v1/health
docker compose -f ~/deploy/iinfo-dx-backend/repo/deploy/production.compose.yaml ps
cat ~/deploy/iinfo-dx-backend/active-slot
```
