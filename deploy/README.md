...
- 저장소: `/home/forestlee/deploy/iinfo/repo` (Actions가 HTTPS로 클론하고 해당 커밋을 체크아웃)
- 개발 설정: `/home/forestlee/deploy/iinfo/env/development.env`
- 운영 설정: `/home/forestlee/deploy/iinfo/env/production.env`
- NPM API 자격증명: `/home/forestlee/deploy/iinfo/npm-credentials.json` (권한 `600`)
- 상태 파일: `/home/forestlee/deploy/iinfo/active-slot`
...
운영 API는 NPM의 `iinfo-dx-api.forestlee.me` 호스트가 현재 슬롯(`iinfo-production-blue` 또는 `iinfo-production-green`)의 8000번 포트로 직접 전달합니다. NPM API 자격증명 파일은 `{"identity":"관리자 이메일","secret":"관리자 비밀번호"}` 형식입니다. 첫 운영 배포에서 프록시 호스트가 없으면 기존 Let's Encrypt `*.forestlee.me` 인증서로 생성합니다. 개발 API는 `iinfo-development-api:8000`이며 서버 내부의 `127.0.0.1:18081`로도 확인할 수 있습니다.
...
docker compose -f ~/deploy/iinfo/repo/deploy/production.compose.yaml ps
cat ~/deploy/iinfo/active-slot
```
